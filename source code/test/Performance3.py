__author__ = 'Akarawit'
from approximation.ApproximationAlgorithmMulti import *
from exact.ExactAlgorithmMulti import *
from datetime import datetime

file_dir = input('Data set: ')
w = int(input('Windows size: '))
r = [float(i) for i in input('Range in each dimension: ').split()]
...
exact_result, exact_n_isb = ExactAlgorithmMulti.run(file_dir=file_dir, w=w, r=r, k=k, s=s)
...
app_n_isb = 0
appro_result = ApproximationAlgorithmMulti.run(file_dir=file_dir, w=w, r=r, k=k, p=p, s=s)
...
print("Number of node in Approximation Algorithm's ISB  unknow" )
# result
